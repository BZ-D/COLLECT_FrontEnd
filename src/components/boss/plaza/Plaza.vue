  ...
  <div class="all-tasks-page">
    ...
    <div style="display: flex; justify-content: center;">
      <el-row type="flex" id="release-btn" style="width: 66%; margin-top: 15px">
        <img src="../../../assets/image/plaza-red.png" alt="" style="width: 150px">
      </el-row>
    </div>
...
<script>
import {getAllValidTasks} from "@/api/task";
...
export default {
  ...
  methods: {
    async getTaskList() {
      const res = await getAllValidTasks();
      if (res.code === 500) {
        this.loading = false;
      ...
      if (res.code === 404) {
        this.loading = false;
        return this.$message.info('未获取到正在进行的任务')
      }
    ...
    gotoDetail(taskid) {
      this.$router.push(`/boss/detail/${taskid}`)
    }
  }
