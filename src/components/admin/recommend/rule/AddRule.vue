<template>
  <div id="add-rule-page">

    <el-main id="rule-detail-box">

      <div>
        <img src="../../../../assets/image/create-rule.png" alt="" style="width: 200px;">
      </div>
      ...
      <div class="button-region">
        ...
        <el-button type="info"
                   @click="$router.push('/admin/recommend/intro')" plain>
          <i class="el-icon-back"></i>&nbsp;返回
        </el-button>
        <el-button type="primary" @click="handleCrete">创建规则</el-button>
      </div>
...
import {addNewRule} from "@/api/recommend-rule"
...
export default {
  ...
  methods: {
    ...
    balanceFactors(changedKey) {
      ...
      const len = Object.keys(this.ruleDetail).length - 1;  // 减一是因为 name 字段占一个位置
      ...
      if (sum > 100) {
        ...
        for (const key of Object.keys(this.ruleDetail)) {
          // 计算每个需要调整属性的步数
          if (key !== 'name' && key !== changedKey && !emptySet.has(key)) {
        ...
        while (diff > 0) {
          for (const key of Object.keys(this.ruleDetail)) {
            if (key !== 'name' && key !== changedKey && !emptySet.has(key) && this.ruleDetail[key] > 0) {
    ...
    handleCrete() {
      this.$confirm('确定创建此条规则吗？', '提示', {
        ...
        const res = await addNewRule(this.ruleDetail);
        if(res.code===200) {
          this.$message({
            duration: 1500,
            type: 'success',
    ...
    checkRules() {
      ...
      for (const key of Object.keys(this.ruleDetail)) {
        if (key === 'name' && this.ruleDetail[key] === '') {
          ...
          return false;
        } else if (key !== 'name' && this.ruleDetail[key] === 0) {
