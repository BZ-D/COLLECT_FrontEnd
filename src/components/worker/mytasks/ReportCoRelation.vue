<template>
  <div>
    ...
    <el-main class="report-relation-outer-box">
      <div class="title">
        <img src="../../../assets/image/report-relation-blue.png" alt="" style="width: 150px; margin-bottom: 20px;">
      </div>
      <el-row>
...
export default {
  ...
  activated() {
    if (this.reportId !== this.$route.query.reportid) {
      this.chart.dispose();
      this.subReportList = [];
      this.getReport();
      this.getSubReports();
  ...
  methods: {
    ...
    async getReport() {
      await getReportByReportID(this.$route.query.reportid).then(res => {
        console.log(res)
    ...
    async getSubReports() {
      await getSubReportsByReportID(this.$route.query.reportid).then(res => {
    ...
    initEcharts() {
      ...
      let nodes = [
        {
          ...
          id: "1",
          url: `/worker/cooperation/evaluate/detail?taskid=${this.$route.query.taskid}&reportid=${this.$route.query.reportid}`,
      ...
      for (const r of this.subReportList) {
        nodes.push({
          ...
          reportId: r.reportId,
          id: id.toString(),
          url: '/worker/mytasks/reportDetail/' + r.reportId,
          category: 1,
          symbolSize: 40
        });
        id++;
      ...
      this.chart.on('click', function (params) {
        if (params.dataType === 'node') {
          that.$store.commit('setReportIdOfSubReport', that.$route.query.reportid);
