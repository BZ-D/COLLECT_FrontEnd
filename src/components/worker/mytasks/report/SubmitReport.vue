    ...
    <div style="display: flex; justify-content: center;">
      <el-row type="flex" style="width: 50%; margin-top: 10px;">
        <el-button icon="el-icon-back"
                   @click="$router.push(`/worker/mytasks/execdetail/${$route.params.taskid}`)"
                   plain
        >
          返回任务</el-button>
      </el-row>
...
import {createScreenshot} from "@/api/report";
...
export default {
  ...
  methods: {
    ...
    async handleSubmitReport(payload) {
      ...
      const res1 = await submitReport(payload);
      if (res1.code === 200) {
