  ...
  <div class="finished-page">
    ...
    <div style="display: flex; justify-content: center;">
      <el-row type="flex" id="release-btn" style="width: 66%; margin-top: 15px">
        <img src="../../../../assets/image/history-blue.png" alt="" style="width: 150px">
      </el-row>
    </div>
    ...
    <div class="list-box">
      ...
      >
        ...
        <el-table-column label="操作" width="130px">
          <template v-slot="scope">
            <el-button type="primary" @click="checkReport(scope.row.id)">查看报告</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
<script>
import {getFinishedTasksByWorkerID} from "@/api/worker";
...
export default {
  ...
  methods: {
    async getTaskList() {
      const res = await getFinishedTasksByWorkerID(sessionStorage.getItem("userid"));
      if (res.code === 500) {
        this.loading = false;
      ...
      if (res.code === 404) {
        this.loading = false;
        return this.$message.info('没有执行完毕的任务');
      }
    ...
    checkReport(taskID) {
      this.$router.push(`/worker/mytasks/report/${taskID}`);
    }
  }
